...
class cifar100_c_testset(torchvision.datasets.CIFAR100):
    ...
    def save_data(self, save_path):
        ...
        torch.save(self.samples, os.path.join(save_path, 'samples.pt'))
        torch.save(self.targets, os.path.join(save_path, 'targets.pt'))

    def load_data(self, load_path):
        self.samples = torch.load(os.path.join(load_path, 'samples.pt'))
        self.targets = torch.load(os.path.join(load_path, 'targets.pt'))
