...
class NCLModelWrapper(torch.nn.Module):
    ...
    def build_ncl_NSdata_wo_con(self, c_r_dir=None):
        if c_r_dir is None:
            c_r_dir = 'configs/NSdata/NSdata.yaml'

        cfg, logger = get_cfg_logger(c_r_dir)
        self.model = get_multi_model_final(cfg, num_classes=10, num_class_list=None, device='cuda', logger=logger)
    ...
    def build_ncl_imagenet_x50_wo_con(self):
        c_r_dir = 'configs/ImageNet_LT/ImageNet_LT_x50_wo_hcm.yaml'
        cfg, logger = get_cfg_logger(c_r_dir)
        self.model = get_multi_model_final(cfg, num_classes=1000, num_class_list=None, device='cuda', logger=logger)
        self.forward = self.forward_wo_con
        return self
